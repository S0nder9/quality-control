...
type UploadPDFsFn = (data: FormData) => Promise<{
    analysis: PageResult[];
...
export const useFileFuncs = (uploadPDFs: UploadPDFsFn): UseFileFuncsReturn => {
    ...
    const handleUpload = useCallback(async () => {
        ...
        try {
            ...
            const { analysis, annotatedFiles } = await uploadPDFs(formData);
            console.log(`Ответ от сервера: ${JSON.stringify({ analysis, annotatedFiles })}`);
